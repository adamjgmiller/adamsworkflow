...
**Cap the set at 7 lenses.** Each lens is a paired Opus + Codex dispatch, and under `/review-fix-loop` this fan-out re-runs every round (see the cost discussion in the *Composition note* below). If more than 7 apply, merge overlapping ones or drop the lowest-value. **State the resolved lens set, its count, and each lens's chosen reviewer model back in one line before dispatching** (`opus`; a purely mechanical lens may drop to `sonnet` — name the reason in a word or two per demotion) — this is also what lets the model-tier rule pass (the orchestrator sizes a known, small fan-out before any dispatch; see Step 3), and it puts each per-lens call on the record instead of leaving it an implicit default.
...
**Capture the baseline first** (anchored to `WORKTREE`): `FANOUT_HEAD=$(git -C "$WORKTREE" rev-parse HEAD)`, the checked-out ref (`git -C "$WORKTREE" symbolic-ref -q HEAD`), and the current dirty set `git -C "$WORKTREE" status --porcelain`. Hold all three.
...
**Opus lens reviewer** (`general-purpose`, one per lens — per-lens model per the model-selection policy: `opus` — a purely mechanical lens may drop to `sonnet`, but never escalate above Opus on your own judgment; a tier the user named overrides that). Brief — compose per lens, folding in that lens's definition:
...
**Worktree-integrity check (after the join, before returning).** Once all lens pairs have returned, diff against the baseline (`FANOUT_HEAD` + checked-out ref + the captured dirty set). The leaves are briefed read-only and Codex is sandboxed read-only, but verify rather than trust. The detection + restore machinery is **canonical in `pr-auto-review.md` Step 7's worktree-integrity paragraph** — the porcelain compare and its limitation (a content-only edit inside an already-dirty path is invisible to it; the read-only briefing is the real guarantee there), `git restore --staged --worktree` for leaks to baseline-clean files (plain `restore` pulls from the index, so a staged leak survives it) plus deletion of leaf-created untracked files, the branch-ref guard before any `reset --soft` (a leaf that switched branches would otherwise have the reset rewrite *that* branch's tip), and collide-with-user-dirt → **stop and flag**, never force-restore. Handle by context:

- **You own the tree** (standalone top-level invocation): actively restore per that canonical machinery, every git command anchored `git -C "$WORKTREE"`.
...
- Capture `FANOUT_HEAD` + checked-out ref + dirty set and run the **same** integrity check here too — the detached Codex is exactly the untrusted external writer the check exists for.
...
4. **Dedup by `(file, ~line, topic)`** — same key `/review-fix-loop` and `/pr-auto-review` use. Findings from multiple sources on the same issue merge into **one** entry. **Attribution is a trailing parenthetical** appended at the end of the merged finding — `(flagged by: opus[security], codex[security])`, naming whichever model was actually dispatched for that lens (e.g. `sonnet[style]`, or `fable[security]` if the user named Fable) — **never** embedded in the one-line summary, Location, or Finding text (that would shift the parse and pollute the normalized `topic`).
...
- **Deepest composed path** (a stage-runner — e.g. build-system's — runs `/review-fix-loop /lens-review` inline): `main(0) → stage-runner(1) → [review-fix-loop inline, +0] → 2a agent(2) → lens children(3)` = **depth 3** (field-notes §5), with the lens fan-out at the leaf level — that's the real ceiling, not 2. (`/pr-auto-review`'s inline loop runs `/dual-review`, which spawns no lens children.)
