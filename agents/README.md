...
3. **Named files are the exception, not the rule.** Per the suite's decision rule: a named
   ...
   pr-auto-review's and build-system's per-lens reviews (`codex-runner` — async like
   every dispatch; collected via its completion task-notification,
   field-notes §4), and (c) a domain-builder role whose def carries a model contract —
